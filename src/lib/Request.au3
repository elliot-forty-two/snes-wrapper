...
;   WITH BUGFIX - DEBUG func enabled
;
; Avaliable Options:
;   Global Options:
;     multi
...
#cs
	...
	$CustomRequest, string, set customrequest parameter for CURL
#ce
...
Func __Request_CurlEasyFinish($Index, $Curl, $Code, $Url, $Opt, $Slist, $HttpPost)
	...
	If IsString($Debug) And IsFunc(Execute($Debug)) Then $Debug = Execute($Debug)
	...
	ElseIf $Code <> $CURLE_OK And IsFunc($Debug) Then
		$Debug(StringFormat("Request error on '%s': %s", $Url, Curl_Easy_StrError($Code)))
		Return SetError(1, $Url & " " & Curl_Easy_StrError($Code))
	EndIf
