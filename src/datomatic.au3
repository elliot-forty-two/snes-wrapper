...
Func CleanROMs()
   ...
   _RunWait("tools\nsrt.exe -savetype uncompressed -remhead -rename -lowext -noext ""*""", "input")
EndFunc
...
Func GetReleaseDate($file, $workingdir = 'input')
...
Func ImportROMs()
   ...
   Local $file
   $files = _FileListToArray("input", "*.s?c", $FLTA_FILES)
   If @error == 0 Then
	  For $i = 1 To $files[0]
...
Func ImportROM($title)
   ...
   Local $data = _RunWait("tools\nsrt.exe -infocsv ""*""", $destdir)
   Local $csv = _CSVRead(StdoutRead($data))
   Local $i, $j
   For $i=1 To UBound($csv) - 1
	  $file =  $csv[$i][0]
	  $company =  $csv[$i][2]
